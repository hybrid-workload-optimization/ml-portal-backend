...
public class ConfigMapService extends ProjectAuthorityService {
	...
	public List<Long> registerConfigMap(ConfigMapDto configMapDto) throws Exception {
		...
		List<Long> result = configMapList.stream()
				.map(c -> {
					...
					try {
						configMapEntity = toConfigMapEntityEntity(cluster, c);
	...
	public ConfigMapDto.Detail getConfigMap(Long configMapIdx, UserDto loginUser) throws Exception {
		...
		ConfigMapDto.Detail dto = ConfigMapDtoMapper.INSTANCE.toDetail(configMapEntity, configMap);
	 ...
	 * @throws Exception
	 */
	public List<Long> updatePersistentVolumeClaim(Long configMapIdx, ConfigMapDto configMapDto) throws Exception {
	 ...
	 * @throws Exception
	 */
	public void deletePersistentVolumeClaim(Long configMapIdx) throws Exception {
		ConfigMapEntity configMapEntity = configMapDomainService.get(configMapIdx);
		...
		if (!isDeleted) {
			throw new PortalException("Config Ma deletion failed");
		}
	 ...
	 * @throws Exception
	 */
	private ConfigMapEntity toConfigMapEntityEntity(ClusterEntity clusterEntity, ConfigMap c) throws PortalException, Exception {
        ...
        //String label		= mapper.writeValueAsString(c.getMetadata().getLabels());
